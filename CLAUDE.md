...
Console-script → module map (in `pyproject.toml`): `tiktok-pipeline`→`tiktok_pipeline`, `tiktok-generate`→`tiktok_image_generator`, `tiktok-upload`→`tiktok_upload`, `tiktok-idea`→`idea_generator`, `tiktok-caption`→`caption_generator`, `tiktok-profile`→`profile_loader`, `imagekit-upload`→`imagekit_uploader`, `airtable-log`→`airtable_logger`, `airtable-migrate`→`airtable_migrate` (each points at the module's `_cli`). Adding a dependency: `uv add <pkg>` (updates `pyproject.toml` + `uv.lock`). There is no test suite or linter config in this repo.
...
**`skills/` packages capabilities as installable Agent Skills (generated, not authored).** The live skills are `tiktok-caption`, `tiktok-image` (generate a PNG only), and `tiktok-upload` (`tiktok_upload.py` — ImageKit upload + Airtable record in one step). `skills/<name>/scripts/` contains COPIES of the root modules — the root modules are the single source of truth, never hand-edit the copies. `skills/sync.sh` regenerates every bundle: it copies each skill's entry module plus the siblings it imports (e.g. `tiktok-upload` bundles `imagekit_uploader.py` + `airtable_logger.py`) and prepends a PEP 723 `# /// script` header to the ENTRY copy only, so `uv run skills/<name>/scripts/<entry>.py` auto-installs deps. After changing any root module, run `bash skills/sync.sh`. The `SKILL.md` files and `skills/README.md` are hand-maintained (sync.sh does not touch them). Skills are consumed by external agents via `npx skills add zazin/tiktok-pipeline/skills/<name>`.
